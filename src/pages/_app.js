...
// ✅ Always mounted at App level — never unmounts during navigation
const CosmicFluidCursor = dynamic(() => import("../components/CosmicFluidCursor"), { ssr: false });
...
function SiteLayout({ children, showLoading, loadingComplete, onLoadComplete }) {
  ...
  return (
    <>
      {/* ✅ showFluidCursor=false — cursor is handled at App level now */}
      <CosmicBackground showFluidCursor={false} />
...
export default function App({ Component, pageProps }) {
  ...
  // ✅ Add cursor-native on mount + watch for it being removed and re-add instantly
  useEffect(() => {
    document.documentElement.classList.add("cursor-native");

    const observer = new MutationObserver(() => {
      if (!document.documentElement.classList.contains("cursor-native")) {
        document.documentElement.classList.add("cursor-native");
      }
    });
  ...
  return (
    <>
      ...
      {/* ✅ Cursor always mounted — never disappears during navigation */}
      {loadingComplete && <CosmicFluidCursor />}
    </>
  );
}